...
const Shop = () => {
  ...
  let allPage = data3.slice(firstPage , lastPage);  /* In this case, Total 30 objects and it will slice from 0 to 6.So there will be 5 items and its length will be 6 */
  ...
  for(let i = 0; 
    i <  Math.ceil( categoryFilter.length > 0 ? categoryFilter : data3.length / perPage) ;
     i++ 
    ){
    pageNumber.push(i);
  }
  ...
  useEffect(() =>{
    setCategory([...new Set(data3.map((item)=> item.category))])
    setBrand([...new Set(data3.map((item)=> item.brand))])
  },[data3])


  let handleCategory = (citem) => {
    let cateFilter = data3.filter((item)=> item.category == citem);
    setCategoryFilter(cateFilter);
  };

  let handleBrand = (bitem) => {
    let brandFilter = data3.filter((item)=> item.brand == bitem);
  ...
  let handlePrice = (value)=>{
    setHigh(value.high);
    setLow(value.low);
    let priceFilter = data3.filter((item)=> item.price > value.low && item.price < value.high)
            ...
            <div className="w-[15%] ">
              ...
              <div>
                ...
                <ul  className="font-dm font-normal text-[16px] text-[#6D6D6D] ">
                  <li onClick={()=>handlePrice({low:0 , high:9})} className="my-[25px]  border-b-[1px] pb-5 hover:font-bold hover:text-black cursor-pointer  ">
                    $0.00 - $9.99
                  </li>
                  <li onClick={()=>handlePrice({low:10 , high:19})} className="my-[25px] border-b-[1px] pb-5 hover:font-bold hover:text-black cursor-pointer ">
                    $10.00 - $19.99
                  </li>              
                </ul>
              </div>
            ...
            <div className="w-[85%] text-center pl-10 ">
              ...
              <div>
                ...
                <div className="text-center pb-20 ">
                  <Pagination                 /* using props to bring pagination's data3 here and vice versa */
